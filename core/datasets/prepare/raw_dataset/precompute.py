...
from prepare.raw_dataset.base import build_streaming_cc12m_precompute_dataloader
from utils import DATA_TYPES, UniversalTextEncoder

"""
Example usage:
accelerate launch --multi_gpu --num_processes 8 precompute.py \
    --datadir ./raw_dataset/mds/ \
    --savedir ./raw_dataset/mds_latents_sdxl1_dfnclipH14/ \
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Local directory with mds shards (output of convert.py).",
    )
    parser.add_argument(
        "--savedir",
        type=str,
...
def main(args):
    ...
    dataloader = build_streaming_cc12m_precompute_dataloader(
        ...
        prefetch_factor=2,
        num_workers=1,  # Reduced from 2 to avoid "Too many open files" with multi-GPU + shared memory
        persistent_workers=True,
        pin_memory=False,  # Disabled to reduce file descriptor usage; re-enable if ulimit is raised
    )
    ...
    # Clear corrupted DINOv2 torch.hub cache if hubconf.py is missing (enables fresh download)
    dinov2_cache = os.path.expanduser("~/.cache/torch/hub/facebookresearch_dinov2_main")
    if accelerator.is_main_process and os.path.isdir(dinov2_cache):
        hubconf_path = os.path.join(dinov2_cache, "hubconf.py")
        if not os.path.isfile(hubconf_path):
            shutil.rmtree(dinov2_cache, ignore_errors=True)
    accelerator.wait_for_everyone()

    visual_encoder = torch.hub.load("facebookresearch/dinov2", f"dinov2_vitb14", trust_repo=True)
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=DATA_TYPES[args.model_dtype]):
                ...
                if f"{cap_key}_attention_mask" in batch:
                    attention_mask = torch.cat(batch[f"{cap_key}_attention_mask"]).to(device)
        ...
        try:
            ...
            for i in range(latents.shape[0]):
                sample_i = batch["sample"][i]
                mds_sample = {
                    cap_key: sample_i[cap_key],
                    ...
                    "image_name": sample_i.get("image_name", ""),
                    "image_path": sample_i.get("image_path", ""),
                }
                if args.save_images:
                    mds_sample["jpg"] = sample_i["jpg"]
