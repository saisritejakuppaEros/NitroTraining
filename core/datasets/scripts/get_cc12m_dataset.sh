...
if [ "$dataset_size" == "small" ]; then
    ...
    python prepare/cc12m/download.py --datadir "${datadir}/wds/" --valid_ids $(seq 0 22) --num_proc $num_proc

# Or download the entire dataset, if specified
elif [ "$dataset_size" == "all" ]; then
    echo "Downloading the full dataset..."
    python prepare/cc12m/download.py --datadir "${datadir}/wds/" --num_proc $num_proc
else
...
python datasets/prepare/cc12m/convert.py --wds_dir "${datadir}/wds/" --local_mds_dir "${datadir}/mds/" \
...
accelerate launch --multi_gpu --num_processes $num_gpus prepare/cc12m/precompute.py --datadir "${datadir}/mds/" \
