...
num_proc=16
batch_size=16 # use batch size of 8 for <16GB GPU memory

# A. Download a small subset (~1%) of the dataset, if specified
if [ "$dataset_size" == "small" ]; then
    echo "Downloading ~1% of the dataset..."
    python prepare/jdb/download.py --datadir $datadir --max_image_size 1024 --min_image_size 1024 --valid_ids 0 1 --num_proc 2
# Or download the entire dataset, if specified
elif [ "$dataset_size" == "all" ]; then
    echo "Downloading the full dataset..."
    python prepare/jdb/download.py --datadir $datadir --max_image_size 1024 --min_image_size 1024 --num_proc $num_proc
else
...
for split in train valid; do
    accelerate launch --multi_gpu --num_processes $num_gpus prepare/jdb/precompute.py --datadir "${datadir}/mds/$split/" \
        --savedir "${savedir}/mds_latents_dcae1024_llama3.2-1b/$split/" --vae mit-han-lab/dc-ae-f32c32-sana-1.0-diffusers \
        --text_encoder meta-llama/Llama-3.2-1B --batch_size $batch_size --image_resolutions 1024
done
