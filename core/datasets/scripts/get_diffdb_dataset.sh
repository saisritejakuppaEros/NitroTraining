...
if [ "$dataset_size" == "small" ]; then
    ...
    python prepare/diffdb/download.py --datadir $datadir --max_image_size 512 --min_image_size 256 \
...
python prepare/diffdb/convert.py --images_dir "${datadir}/raw/" --local_mds_dir "${datadir}/mds/" \
...
accelerate launch --multi_gpu --num_processes $num_gpus prepare/diffdb/precompute.py --datadir "${datadir}/mds/" \
